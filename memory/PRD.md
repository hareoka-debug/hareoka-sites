...
- [x] Fotos reales georreferenciadas en los 41 POIs: campo "photo" en routes_data.py con imágenes reales de Wikimedia Commons (verificadas 200 OK, thumbs 1280px). UI: miniatura en cada POI + coordenadas siempre visibles; tap expande foto grande con badge "Foto real · coordenadas".
- [x] Ícono oficial de la app: moái estilizado vectorial (PIL, paleta terracota/arena/océano, script /tmp/make_icon.py) aplicado a icon.png, adaptive-icon.png (Android, bg #F5EFE6), favicon.png y splash-image.png. app.json: name "Descubre Rapa Nui", splash y adaptiveIcon backgroundColor #F5EFE6. NOTA: el ícono/splash solo se ven en builds reales (no en Expo Go). Emergent LLM key sin saldo (usuario eligió ícono por código en vez de IA).
